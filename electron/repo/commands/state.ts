/**
 * electron/repo/commands/state.ts — Commands 模块 enabled 状态
 *
 * v5 wave-3 改造(2026-07-30 PRD real-disable):
 * 用户的 enabled toggle = 改 ~/.claude/commands/<name>.md → <name>.md.disabled
 * (PRD 决策:Claude Code 不读 .disabled 后缀文件)。
 *
 * mcp_server_state KV 表(cmd:enabled:<name>)仍保留作 cache +
...
import { setDisabledSuffix as writeCommandsSuffix } from '../settings-writer';
...
export async function setEnabled(
  ...
  await writeCommandsSuffix('command', name, !enabled, baseDir);
  try {
