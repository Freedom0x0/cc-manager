/**
 * electron/repo/skills/state.ts — Skills 模块 enabled 状态 KV 助手
 *
 * 复用 mcp_server_state 表(CLAUDE.md v5 D1 决策延伸):3 列 KV 模型
 * (key PRIMARY KEY / value / updated_at)通用,key 前缀隔离:
 *   - 'skill:enabled:<name>'       → 'true' / 'false' (用户 toggle)
 *   - 'skill:last_modified:<name>' → ISO timestamp
 *
...
/** 读 enabled 状态。无 key 默认 true(全新 skill 视为启用) */
