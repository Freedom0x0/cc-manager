/**
 * electron/repo/plugins/state.ts — Plugins 模块 enabled 状态 KV 助手
 *
 * 复用 mcp_server_state 表(CLAUDE.md v5 D1 / D6 决策延伸):3 列 KV 模型
 * (key PRIMARY KEY / value / updated_at)通用,key 前缀隔离:
 *   - 'plugin:enabled:<name>' → 'true' / 'false' (用户 toggle)
 *
 * 注意:虽然表名是 mcp_server_state,但本质是带 key 前缀隔离的通用 KV
 * 命名空间。Plugins 跟 MCP/Skills/Commands/Sub-Agents/Hooks 复用同一张表,
...
/** 读 enabled 状态。无 key 默认 true(全新 plugin 视为启用) */
export function getEnabled(db: DB, name: string): boolean {
  const row = stmts(db).select.get(`plugin:enabled:${name}`) as { value: string | null } | undefined;
...
export function setEnabled(db: DB, name: string, enabled: boolean): void {
  stmts(db).upsert.run(`plugin:enabled:${name}`, enabled ? 'true' : 'false', Date.now());
}
