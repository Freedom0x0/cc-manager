 ...
 *
 * Case 1: src/api.ts 7 函数包装都导出
 *   - 验证 api.skillList / skillGet / skillCreate / skillUpdate / skillDelete /
 *     skillToggleEnabled / skillImportFile 都存在(类型 = function)。
 *     漏一个 = UI 调用会运行时崩。
 *
 ...
 *
 * Case 3: src/mock.ts 7 mock 实现都存在
 *   - 验证浏览器 dev 模式(纯 vite serve)能正常加载 fixture 数据。
 *
 * 为什么 3 case 不动 IPC handler / 主进程?skills.test.ts(6 case)
...
// Case 1: api 7 包装函数都导出且都是函数
test('api.skill* — 7 wrapper functions are exported', () => {
  const expected = [
    'skillList',
    'skillGet',
    'skillCreate',
...
// Case 3: mock.ts 7 mock 实现都存在
test('mock.ts implements 7 skill* methods (browser dev fixture)', () => {
