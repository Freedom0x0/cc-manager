...
// Case 2: fixture 目录 → 返 fixture command + enabled 注入
test('listCommands reads fixture directory and injects enabled state from KV', async () => {
  ...
  assert.strictEqual(c.enabled, true, '默认 enabled=true(KV 表无 key)');

  // 设 enabled=false 后应读出来
  setEnabled(db, 'review', false);
